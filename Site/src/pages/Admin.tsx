              ...
              {activeTab === 'settings' && (
                <div className="space-y-8 animate-in fade-in duration-500">
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    ...
                    {isMaster && (
                      <div className="p-6 bg-amber-50 border border-amber-200 rounded-2xl space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-sm font-bold text-amber-700 mb-2">Valor Metro Quadrado Venda (R$)</label>
                            <input type="number" step="0.01" value={settings.pricePerM2 || ''} onChange={e => setSettings({ ...settings, pricePerM2: e.target.value })}
                              className={inputCls} />
                          </div>
                          <div>
                            <label className="block text-sm font-bold text-amber-700 mb-2">Valor Custo Calha (R$)</label>
                            <input type="number" step="0.01" value={settings.costPerM2 || ''} onChange={e => setSettings({ ...settings, costPerM2: e.target.value })}
                              className={inputCls} />
                          </div>
                          <div>
                    ...
                    {/* Automação de WhatsApp e Validade */}
                    {isAdmin && (
                      <div className="p-6 bg-green-50 border border-green-200 rounded-2xl space-y-6">
                        <div className="flex items-center justify-between">
                          <h3 className="font-bold text-green-800 flex items-center gap-2">
                            <MessageSquare className="w-5 h-5" /> Automação de WhatsApp e Validade
                          </h3>
                          <button
                            type="button"
                            onClick={async () => {
                              const dest = prompt('Digite o número para o teste (Ex: 5566999991122):');
                              if (!dest) return;
                              const res = await fetch('/api/whatsapp/test-connection', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ phone: dest, settings }),
                                credentials: 'include'
                              });
                              if (res.ok) alert('Tentativa enviada! Veja se chegou no WhatsApp escolhido.');
                              else alert('Falha ao enviar. Verifique o console do servidor.');
                            }}
                            className="bg-white text-green-700 px-4 py-2 rounded-xl text-xs font-black border border-green-200 shadow-sm hover:bg-green-100 transition-all cursor-pointer">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            <label className="block text-sm font-bold text-green-800 mb-2">Validade Padrão do Orçamento (Dias)</label>
                            <input type="number" value={settings.defaultValidadeDays || '7'}
                              onChange={e => setSettings({ ...settings, defaultValidadeDays: e.target.value })}
                              className={inputCls} />
                            <p className="text-[10px] text-green-600 mt-1">Após este prazo, orçamentos 'Enviados' mudam para 'Expirado'.</p>
                          </div>
                          <div>
                            <label className="block text-sm font-bold text-green-800 mb-2">Habilitar Robô de WhatsApp?</label>
                            <select
                              value={settings.whatsappAutomationEnabled || 'false'}
                              onChange={e => setSettings({ ...settings, whatsappAutomationEnabled: e.target.value })}
                              className={inputCls}
                            >
                              <option value="false">Não - Desativado</option>
                              <option value="true">Sim - Ativar Follow-up Automático</option>
                            </select>
                          </div>
                        </div>

                        {settings.whatsappAutomationEnabled === 'true' && (
                          <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                            <div className="p-4 bg-white rounded-xl border border-green-100 space-y-3">
                              <p className="text-xs font-black text-green-700 uppercase">Configuração da API (Evolution API)</p>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <input placeholder="URL completa de envio (incluindo instance)" value={settings.whatsappApiUrl || ''}
                                  onChange={e => setSettings({ ...settings, whatsappApiUrl: e.target.value })} className={inputCls} />
                                <input placeholder="API Key / apikey" type="password" value={settings.whatsappApiKey || ''}
                                  onChange={e => setSettings({ ...settings, whatsappApiKey: e.target.value })} className={inputCls} />
                              </div>
                              <p className="text-[10px] text-slate-400 italic">Dica: URL deve ser algo como: https://sua-api.com/message/sendText/NOME_INSTANCIA</p>
                            </div>

                            <div className="space-y-3">
                              <p className="text-xs font-black text-green-700 uppercase">Personalizar Mensagens</p>
                              <p className="text-[10px] text-slate-500">Use {'{cliente}'} para o nome e {'{id}'} para o número do orçamento.</p>
                              <div>
                                <label className="text-[10px] font-bold text-slate-500 block mb-1">Lembrete 1 dia após envio</label>
                                <textarea rows={2} value={settings.whatsappMsgLembrete || ''}
                                  onChange={e => setSettings({ ...settings, whatsappMsgLembrete: e.target.value })}
                                  placeholder="Olá {cliente}, notamos que seu orçamento {id} ainda está pendente..." className={inputCls} />
                              </div>
                              <div>
                                <label className="text-[10px] font-bold text-slate-500 block mb-1">Aviso 1 dia antes de expirar</label>
                                <textarea rows={2} value={settings.whatsappMsgAnteExpiracao || ''}
                                  onChange={e => setSettings({ ...settings, whatsappMsgAnteExpiracao: e.target.value })}
                                  placeholder="Aproveite para fechar seu pedido {id} antes que o orçamento expire amanhã!" className={inputCls} />
                              </div>
                            </div>
                          </div>
                        )}
